<?php /*a:7:{s:83:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/portal/list_img_sidebar.html";i:1769524402;s:71:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/public/head.html";i:1765520921;s:75:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/public/function.html";i:1768462231;s:73:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/public/config.html";i:1759133767;s:70:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/public/nav.html";i:1768899266;s:73:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/public/footer.html";i:1769077938;s:74:"/www/wwwroot/www.hyperionrobot.com/public/themes/robai/public/scripts.html";i:1768562960;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<title><?php echo $category['name']; ?> <?php echo $category['seo_title']; ?> <?php echo (isset($theme_vars['site_name']) && ($theme_vars['site_name'] !== '')?$theme_vars['site_name']:''); ?></title>
...
<script src="/static/js/wind.js"></script>
 ...
 ?>
		<style type="text/css" media="all">
			.courses-area.trend-layout .courses-items .bottom-info ul li{font-size:12px;}
            .info h5 a{font-size:14px;font-weight:bold;}
            .sideber-title{border-bottom:1px solid #dfdfdf;font-size:15px;font-weight:bold;padding:22px 30px;color:#666;margin:0;}
            .accordion{padding:21px 15px 21px 30px;}
            .card-box .header button{background:none;font-weight:normal;display:flex;justify-content:space-between;align-items:center;padding:6px;}
            .card-box .header button:hover{color:#ff1949;}
            .card-box .header > button > i{font-size:12px;}
            .sideber-ul li{padding:6px 6px 6px 15px;position:relative;margin-left:15px;}
            .sideber-ul li a{font-weight:normal;font-size:14px;color:#666;}
            .sideber-ul li a.active{color:#ff1949;}
            .sideber-ul li::before{position:absolute;display:inline-block;content:"";width:3px;height:3px;background:#666;border-radius:50%;left:5px;line-height:20px;top:48%;}
		</style>
	</head>
	<body>
		<!-- Start Header Top 
...
<!-- End Header -->
		...
		<!--<div class="breadcrumb-area bg-gray text-center shadow dark text-light bg-cover"
			style="background-image: url(https://src.hyperionrobot.com/hyper-web/banner_case_video.jpg);">
			<div class="container">
				<div class="row">
		...
		<!-- Star Courses Area
        ============================================= -->
		<div class="courses-area trend-layout  default-padding" style="background-color: #f7f9fa;">
			<div class="container">
				<?php 
					$categoryId = $category['id'];
					
					$parentId = _wien_parentCategoriesId($category['id']);
					
					//echo $parentId;
					
					if($parentId==0){
					    $subCats = _wien_subCategories($categoryId);
					     
					}else{
                        $par = _wien_parentCategoriesId($parentId);
                        
    					$subCats = _wien_subCategories($par);
    					$categoryId = $par;
    					$parent = _wien_getCategoryName($parentId);
    					$parName = _wien_getCategoryName($par);
					}
				    //echo $subCats;
				 ?>
				<div class="row">
					<div class="col-md-3">
					    <div style="background-color: #fff; border-radius: 10px; border: 1px solid #dfdfdf;">
    						<h4 class="sideber-title">
    							<?php if($parentId==0): ?><?php echo $category['name']; else: if(!(empty($parName) || (($parName instanceof \think\Collection || $parName instanceof \think\Paginator ) && $parName->isEmpty()))): ?>
    									    	<?php echo $parName['name']; ?>
    									<?php endif; ?>
    						...
    						<div class="accordion" id="accordionExample">
    						    <?php if(is_array($subCats) || $subCats instanceof \think\Collection || $subCats instanceof \think\Paginator): if( count($subCats)==0 ) : echo "" ;else: foreach($subCats as $key=>$vo): ?>
    							<div class="card-box">
    								<div class="header" id="headingOne">
    								
    										<button class="btn-block text-left" type="button"
    											data-toggle="collapse" data-target="#collapse-<?php echo $key+1; ?>" aria-expanded="true"
    								...
    								<div id="collapse-<?php echo $key+1; ?>" class="collapse show" aria-labelledby="headingOne"
    									data-parent="#accordionExample">
    									<div class="body">
    										<ul class="sideber-ul">
    										    
    										    <?php $subCatsAll = _wien_subCategories($vo['id']); if(is_array($subCatsAll) || $subCatsAll instanceof \think\Collection || $subCatsAll instanceof \think\Paginator): if( count($subCatsAll)==0 ) : echo "" ;else: foreach($subCatsAll as $key=>$voo): ?>
                    							<li><a href="<?php echo cmf_url('portal/List/index',array('id'=>$voo['id'])); ?>"><?php echo $voo['name']; ?></a></li>
                    							<?php endforeach; endif; else: echo "" ;endif; ?>
                    						</ul>
    									</div>
    								</div>
										...
										<div class="single-item col-lg-4 col-md-6">
											<div class="item">
												<div class="thumb">
													<a href="javascript:;">
														<?php if(empty($vo['more']['thumbnail'])): ?>
															<img src="/themes/robai/public/assets/img/common/picture_2.png"
																alt="Thumb">
															<?php else: ?>
												...
												<div class="info" style="padding:15px">
													<h5>
														<a href="javascript:;"><?php echo $vo['post_title']; ?></a>
													</h5>
													<p>
														...
														<div class="course-info">
															<ul>
																<li><i class="fas fa-clock"></i>
																	<?php echo date('Y-m-d',$vo['published_time']); ?></li>
																<li><i class="fas fa-list-ul"></i> <?php echo $vo['post_like']; ?></li>
																<li><i class="fas fa-user"></i> <?php echo $vo['post_hits']; ?></li>
															</ul>
														</div>
														<div class="enroll">
															<?php if(empty($vo['more']['video'])): ?>
																<a href="javascript:;"
																	class="btn btn-theme effect btn-sm"
																	style="background-color:#5f5f5f;border-color:#5f5f5f;padding:5px;">立即播放</a>
																<?php else: ?>
																<a href="<?php echo cmf_get_image_url($vo['more']['video']); ?>"
																	class="btn btn-theme effect btn-sm popup-youtube light item-center"
																	style="padding:5px;">立即播放</a>
															<?php endif; ?>

															<!--<a class="btn btn-theme effect btn-sm" href="<?php echo cmf_url('portal/Article/index',array('id'=>$vo['id'])); ?>">立即播放</a>-->
														</div>
													</div>
												</div>
											</div>
