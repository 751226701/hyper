...
class VerificationCodeController extends HomeBaseController
{
    public function send3()
    {
        ...
        if ($accountType == 'email') {
            ...
            $params = [
                'phoneNumbers' => array($data['username']), // 必填，接收短信电话号码，数组。
                'templateParam' => [strval($code)] // 选填，如果模板中有变量，请严格按模板设置顺序准备。
            ];
            // 调用钩子并获取返回结果
...
//            'captcha'  => 'require',
        ...
        if ($accountType == 'email') {
            ...
            $params = [
                'phoneNumbers'  => ['18220523738', '18049081884'],
                'templateParam' => [ // 可选项，只有模板中有变量时使用
                    'code'    => $code
                ],
                 'templateId'     => '2538286'
