...
// +----------------------------------------------------------------------
namespace app\discuss\taglib;

use think\template\TagLib;

class Discuss extends TagLib
{
    /**
     * 定义标签列表
     */
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::articles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::tagArticles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagPage($tag, $content)
    {
        ...
        $returnVarName = empty($tag['item']) ? 'discuss_page' : $tag['item'];

        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::page({$id});
?>
{$content}
parse;
        return $parse;
...
<?php
if(!empty({$cid})){
    \$__BREADCRUMB_ITEMS__ = \app\discuss\service\ApiService::breadcrumb({$cid},{$self});
?>
...
parse;
    ...
    public function tagCategories($tag, $content)
    {
        ...
        $returnVarName = 'discuss_categories_data';
        if (strpos($ids, '$') === 0) {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}',
    'ids'     => {$ids}
]);
...
parse;
    ...
    public function tagCategory($tag, $content)
    {
        ...
        $returnVarName = empty($tag['item']) ? 'discuss_category' : $tag['item'];

        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::category({$id});
?>
{$content}
parse;
        return $parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'discuss_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::subCategories({$categoryId});
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'discuss_all_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\discuss\service\ApiService::allSubCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
