...
class DiscussCategoryModel extends Model
{
    /**
     * 模型名称
     * @var string
     */
    protected $name = 'discuss_category';
    ...
    {
        $total = DiscussCategoryPostModel::where('category_id', $data['id'])->where('status', 1)->count();
        return $total;
    }
...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['review']        = empty($item['review']) ? '<span class="label label-warning">不审核</span>' : '<span class="label label-success">审核</span>';
            $item['checked']        = in_array($item['id'], $currentIds) ? "checked" : "";
            $item['url']            = cmf_url('discuss/List/index', ['id' => $item['id']]);
    ...
    public function addCategory($data)
    {
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'discuss/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'discuss/Article/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data)
    {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            if (!empty($categoryAlias)) {
                $routeModel->setRoute($categoryAlias, 'discuss/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($categoryAlias . '/:id', 'discuss/Article/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('discuss/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('discuss/Article/index', ['cid' => $data['id']]);
            }
