...
class TopicService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        $articlesQuery   = $discussPostModel->alias('a');
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $article = $discussPostModel->alias('post')->field('post.*')
                ->where($where)
            ...
            $article = $discussPostModel->alias('post')->field('post.*')
                ->join('discuss_category_post relation', 'post.id = relation.post_id')
                ->where($where)
                ->where($wherePublishedTime)
                ->find();
        }
