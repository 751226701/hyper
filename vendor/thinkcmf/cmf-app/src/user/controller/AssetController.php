...
class AssetController extends AdminBaseController
{
    ...
    public function vditor()
    {
        if ($this->request->isPost()) {
            ...
            if(isset($result['url'])){
                ...
                return json(['msg'=>'上传成功！','code'=>1,'data'=>$result]);
            }else{
                return json(['msg'=>'上传失败！','code'=>0,'data'=>[]]);
            }
        }
    }
    ...
    public function upload()
    {
        if ($this->request->isPost()) {
            ...
            $uploader = new Upload();
            $uploader->setUploadDir(CMF_DATA . 'upload/tmp/');
            $uploader->setPrivate(true);
            $result = $uploader->upload();
